...
public class NinjaAPI implements NinjaAPILocal, NinjaAPIRemote {
	...
	private static final String [] bryterPåKode = {
	...
	public boolean switchState[] = new boolean [bryterPåKode.length];
    ...
    public void settBryter(int bryterId, boolean på) throws IOException
    {

    	
    	if (bryterId < bryterPåKode.length)
    	{
    		String kode;
    		if (på)
    		{
    			kode = bryterPåKode[bryterId];
    		}
    		else {
    			kode = bryterAvKode[bryterId];
    		} 
    		switchState[bryterId] = på;
    		sendKode(kode);
    	}
    }
	...
	public void Callback(String deviceGuid, String id, long timestamp) {
		if (RF_GUID.equals(deviceGuid))
		{
			...
			for (int i=0; i<bryterPåKode.length; i++)
			{
				bryterKode = bryterKode || bryterPåKode[i].equals(id) || bryterAvKode[i].equals(id);  
			}
